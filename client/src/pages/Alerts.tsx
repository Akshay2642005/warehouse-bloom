...
export default function Alerts() {
  ...
  const criticalCount = alerts.filter(item => item.quantity === 0).length;
  const lowStockCount = alerts.filter(item => item.quantity > 0 && item.quantity < 10).length;
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-info">{alerts.length}</p>
              </div>
            ...
            <div className="space-y-4">
              {alerts.map((item) => (
                <div key={item.id} className="flex items-start gap-4 p-4 border rounded-lg hover:bg-muted/50 transition-colors">
                  <div className="flex-shrink-0 mt-1">{getStatusIcon(item.quantity)}</div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-medium text-sm">Low Stock Alert</span>
                      {getSeverityBadge(item.quantity)}
                    </div>
                    <p className="text-foreground mb-2">{item.name} (SKU: {item.sku}) - {item.quantity === 0 ? 'Out of stock' : `Only ${item.quantity} units remaining`}</p>
                    <p className="text-sm text-muted-foreground">Price: ${(item.priceCents / 100).toFixed(2)}</p>
                  </div>
